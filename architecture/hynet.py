...
class HyNet(nn.Module):
    ...
    def forward(self, x, is_training=False, opts=None):
        for layer in [self.layer1, self.layer2, self.layer3, self.layer4]:
            x = layer(x)

        if opts is not None:
            weight_adv = opts["weight_adv"].view(1, -1, 1, 1)
            bias_adv = opts["bias_adv"].view(1, -1, 1, 1)
            x = weight_adv * x + bias_adv
        for layer in [self.layer5, self.layer6, self.layer7]:
            x = layer(x)
    ...
    def get_feature_map(self, x):
        x1 = self.layer1(x)  # bs * 32 * 32 * 32
        x2 = self.layer2(x1)  # bs * 32 * 32 * 32
        x3 = self.layer3(x2)  # bs * 64 * 16 * 16
        x4 = self.layer4(x3)  # bs * 64 * 16 * 16
        x5 = self.layer5(x4)  # bs * 128 * 8 * 8
        x6 = self.layer6(x5)  # bs * 128 * 8 * 8
